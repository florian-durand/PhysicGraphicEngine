...
{
    std::vector<VkDescriptorSetLayout> layouts(data.MAX_FRAMES_IN_FLIGHT, descriptorSetLayout);
    VkDescriptorSetAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocInfo.descriptorPool = descriptorPool;
    allocInfo.descriptorSetCount = static_cast<uint32_t>(data.MAX_FRAMES_IN_FLIGHT);
    allocInfo.pSetLayouts = layouts.data();
    descriptorSets.resize(data.MAX_FRAMES_IN_FLIGHT);
...
void RenderApplication::createDescriptorPool()
{
    ...
    poolSize.descriptorCount = static_cast<uint32_t>(data.MAX_FRAMES_IN_FLIGHT);
    ...
    poolInfo.maxSets = static_cast<uint32_t>(data.MAX_FRAMES_IN_FLIGHT);
...
void RenderApplication::createUniformBuffers()
{
    ...
    uniformBuffersMapped.resize(data.MAX_FRAMES_IN_FLIGHT);
...
void RenderApplication::cleanupSwapChain()
{
...
void RenderApplication::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
...
{
    imageAvailableSemaphores.resize(data.MAX_FRAMES_IN_FLIGHT);
    renderFinishedSemaphores.resize(data.MAX_FRAMES_IN_FLIGHT);
    inFlightFences.resize(data.MAX_FRAMES_IN_FLIGHT);
...
{
    commandBuffers.resize(data.MAX_FRAMES_IN_FLIGHT);
...
VkFormat RenderApplication::findDepthFormat()
{
...
VkCommandBuffer RenderApplication::beginSingleTimeCommands()
{
    VkCommandBufferAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
...
void RenderApplication::endSingleTimeCommands(VkCommandBuffer commandBuffer)
{
    vkEndCommandBuffer(commandBuffer);
...
void RenderApplication::createDepthResources()
{
...
VkImageView RenderApplication::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags)
{
...
RenderApplication::SwapChainSupportDetails RenderApplication::querySwapChainSupport(VkPhysicalDevice device)
{
...
bool RenderApplication::QueueFamilyIndices::isComplete()
{
    return graphicsFamily.has_value() && presentFamily.has_value();
}

RenderApplication::QueueFamilyIndices RenderApplication::findQueueFamilies(VkPhysicalDevice device)
{
...
VkSurfaceFormatKHR RenderApplication::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR> &availableFormats)
{
...
VkPresentModeKHR RenderApplication::chooseSwapPresentMode(const std::vector<VkPresentModeKHR> &availablePresentModes)
{
...
VkExtent2D RenderApplication::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities)
{
